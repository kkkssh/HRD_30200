...
select stuid, sname, substr(jumin,1,7)||'-'||substr(jumin,7,13) jumin, dept_name, substr(jumin,7,1) gender, phone, email
from tbl_student_202210;

select t.stuid,sname, subname, c.subcode, proname, midscore, finalscore, attend, report, etc, 
...
select t.stuid, sname, dept_name, subname, midscore, finalscore, attend, report, etc,	
	to_char((midscore*0.3) + (finalscore*0.3) + (attend*0.2)+(report*0.1)+(etc*0.1),'999.9') sum,
	(midscore*0.3) + (finalscore*0.3) + (attend*0.2)+(report*0.1)+(etc*0.1) grade
from tbl_score_202210 c, tbl_subject_202210 u, tbl_student_202210 t
where c.subcode = u.subcode
